...
#   plot-vcfstats vcf_comparison_results/stats/combined_stats.txt -p vcf_comparison_results/plots --no-PDF
#
# Edit as necessary and recreate the plots by running
#   python3 plot.py
#
# Title abbreviations:
# 	 0 .. submi .. Terra/submissions_1717f5fa-7b6c-4312-95b0-801562a1bb6b_SRWholeGenome_0fae6438-5610-4b17-8a39-3b4ec2552d28_call-ScoreIndelVariantAnnotations_S3_WGS-batchC_4051002301-ST140_S9_ALL_scored.vcf.gz
# 	 1 .. combi .. vcf_comparison_results/combined_karamoko.vcf.gz
# 	 2 .. submi + combi .. Terra/submissions_1717f5fa-7b6c-4312-95b0-801562a1bb6b_SRWholeGenome_0fae6438-5610-4b17-8a39-3b4ec2552d28_call-ScoreIndelVariantAnnotations_S3_WGS-batchC_4051002301-ST140_S9_ALL_scored.vcf.gz
#
...
if plot_venn_snps:
    ...
    ax1.bar([1,2,3],[1789,0,1473],align='center',color='orange',width=0.3)
    ax1.ticklabel_format(style='sci', scilimits=(0,0), axis='y')
    ax1.set_xlim(0.5,3.5)
    plt.xticks([1,2,3],('submi\nts/tv 0.73\n1,789','submi + combi\nts/tv 0.00\n0','combi\nts/tv 0.38\n1,473'))
...
if plot_venn_indels:
    fig = plt.figure(figsize=(4.33070866141732,3.93700787401575))
    ax1 = fig.add_subplot(111)
    ax1.bar([1,2,3],[2252,0,2068],align='center',color='red',width=0.3)
    ax1.ticklabel_format(style='sci', scilimits=(0,0), axis='y')
    ax1.set_xlim(0.5,3.5)
    plt.xticks([1,2,3],('submi\n2,252','submi + combi\n0','combi\n2,068'))
...
if 0 in dat and len(dat[0])>2:
    ax1.plot([row[0] for row in dat[0]], [row[1] for row in dat[0]], '-o',markersize=3, color='orange',mec='orange',label='submi')
    has_data = 1
        
if 1 in dat and len(dat[1])>2:
    ax1.plot([row[0] for row in dat[1]], [row[1] for row in dat[1]], '-o',markersize=3, color='red',mec='red',label='combi')
    has_data = 1
        
if 2 in dat and len(dat[2])>2:
    ax1.plot([row[0] for row in dat[2]], [row[1] for row in dat[2]], '-o',markersize=3, color='darkgreen',mec='darkgreen',label='submi + combi')
    has_data = 1
...
if plot_SNP_overlap_by_af and len(dat)>1:
    ...
    ax1.set_ylabel('Fraction found in combi [%]')
    ax1.set_xscale('log')
    ax1.set_xlabel('Non-reference allele frequency in submi')
    ax1.set_xlim(0,1.01)
...
if plot_tstv_by_qual and len(dat)>2:
    fig = plt.figure(figsize=(4.33070866141732,3.93700787401575))
    ax1 = fig.add_subplot(111)
    ax1.plot([row[1] for row in dat], [row[3] for row in dat], '-', ms=1, mec='red', color='red', label='Cumulative ts/tv')
    ax1.plot([row[1] for row in dat], [row[2] for row in dat], '--', ms=1, mec='red', color='red', label='Per 1% bins')
    ax1.set_ylabel('Ts/Tv',fontsize=10)
    ax1.set_xlabel('Number of sites\n(sorted by QUAL, descending)',fontsize=10)
    ax1.ticklabel_format(style='sci', scilimits=(-3,2), axis='x')
    ax1.set_ylim(min(2,min(row[2] for row in dat))-0.3,0.3+max(2.2,max(row[2] for row in dat)))
...
dat = []
with open('indel_vaf.1.dat', 'r') as f:
    reader = csv.reader(f, 'tab')
    for row in reader:
        if row[0][0] != '#': dat.append([float(x) for x in row])
...
if plot_indel_vaf and len(dat)>0:
    fig = plt.figure(figsize=(4.33070866141732*2,3.93700787401575*0.7))
    ax1 = fig.add_subplot(111)
    ax1.plot([-60,51], [0.5,0.5],color='#c5c5c5')
    ax1.plot([row[0] for row in dat], [row[1] for row in dat],'.-',color='orange')# , edgecolor='orange')
    ax1.set_xlabel('Size of deletion (negative) or insertion (positive)')
    ax1.set_ylabel('Fraction of alt allele')
    ax1.ticklabel_format(style='sci', scilimits=(0,0), axis='y')
    ...
    plt.savefig('indel_vaf.1.png')
    if img_fmt != 'png': plt.savefig('indel_vaf.1.' + img_fmt)
    plt.close()
